#!/usr/bin/env python3
"""Training script for Correlated Q-Learning on Cooperative Box Pushing.

This script trains two agents using Correlated Q-Learning to solve the
...
def create_agents(env):
    """Create two Correlated Q-Learning agents."""
    num_players = env.num_players
    num_actions = [env.action_spec()['num_actions']] * num_players
...
def train_agents(env, agents, num_episodes=10000, eval_every=1000):
    """Train the agents using Correlated Q-Learning."""
    ...
    for episode in range(num_episodes):
        ...
        while not time_step.last():
            ...
            for i, agent in enumerate(agents):
                step_output = agent.step(time_step, actions, is_evaluation=False)
                new_actions.append(step_output.action)
...
def evaluate_agents(env, agents, num_eval_episodes=100):
    ...
    for episode in range(num_eval_episodes):
        ...
        while not time_step.last():
            ...
            new_actions = []
            for i, agent in enumerate(agents):
                step_output = agent.step(time_step, actions, is_evaluation=True)
                new_actions.append(step_output.action)
...
if __name__ == "__main__":
    ...
    episode_rewards, episode_lengths, success_count = train_agents(
        env, agents, num_episodes=10000, eval_every=1000
    )
